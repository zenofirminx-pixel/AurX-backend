...
${messages[1].content}
    ...
    // FORMAT REPLY
    // =========================
    let finalReply = formatReply(rawReply);

    // =========================
    // 🔗 LINK ENGINE INTEGRATION
    // =========================
    const lowerMessage = message.toLowerCase();

    if (
      lowerMessage.includes("lien") ||
      lowerMessage.includes("link") ||
      lowerMessage.includes("partage") ||
      lowerMessage.includes("share")
    ) {
      const autoLink = generateLink("chat", Date.now());

      finalReply += `\n\n🔗 Lien généré : ${autoLink}`;
    }
