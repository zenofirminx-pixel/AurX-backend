...
async function cleanupOldData(db, userId) {
  ...
  if (convSnap.exists) {
    ...
    const filtered = conversations.filter(c => {
      const last = c.updatedAt || c.date || 0;
...
export default async function handler(req, res) {
  try {
    ...
    // 🚀 ONLY SYSTEM PROMPT (NO MIXING HERE)
    const systemPrompt = buildPrompt(memoryText);
    ...
    cleanupOldData(db, userId).catch(() => {});
